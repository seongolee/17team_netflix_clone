...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        print(username)
        user_phone = request.POST.get('username', None)
        ...
        me_username = auth.authenticate(request, username=user_email, password=password)
        print(me_username)
        print('유저네임 로그인 권한 받았습니다')
        me_phone_number = auth.authenticate(request, phone_number=username, password=password)
        print('로그인 권한 받았습니다')
        print(me_username)
